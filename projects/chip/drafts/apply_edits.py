...
ACRES = 'Tolles decision 8/20 (200-ft corridor straight over the corner): wings of ~141-ft legs and ~200-ft outer face = ~0.23 ac each; 7 wings = ~1.6 ac. The 0.51-ac wing figure carried over from the superseded geometry. EPS/civil: remap KMZ and confirm the 200-ft corridor carries the road and the 34.5 kV MV lines.'
ROAD = 'Tolles 9/25: the Hart Lane road follows the existing two-lane gravel road on Reclamation land (~2,750 ft), ends at the CHIP property line, then runs around the site perimeter on private land. Needs legal description, width, acreage, and confirmation of the gravel road\'s legal status (county ROW or not). Paving an existing alignment supports the minor-and-localized factors (previously disturbed land).'
...
R.edit(206, 'This PPOD and the accompanying SF-299 address four corner crossings and an access road segment on Reclamation-administered land, described in Section 1.1.', FMDP)
...
rep(219, 'The four crossings comprise seven wings and occupy approximately 3.57 acres in total, on land administered by Reclamation as part of the Newlands Project.',
    'The four crossings comprise seven wings and occupy approximately 1.6 acres in total at the current 200-foot design standard (Section 3), on land administered by Reclamation as part of the Newlands Project. The Project also includes an access road segment of approximately 2,750 feet on Reclamation-administered land that improves an existing gravel road on the Hart Lane alignment, described below.', ACRES)
t = T(220)
...
R.edit(223, 'Hart Lane access road segment. The Project includes an access road segment of approximately 2,750 feet on Reclamation-administered land, continuing the Hart Lane alignment from its current end to the western boundary of the CHIP parcels. The segment follows an existing two-lane gravel road that has been in use for many years. The proposed improvement would pave and improve the existing alignment within a road corridor of [INSERT: road right-of-way width] feet, so that most of the associated ground disturbance would occur on previously disturbed land. At the CHIP property line the road would continue on private land around the perimeter of the development and would not occupy additional Reclamation-administered land. Construction use of the existing gravel road prior to its improvement is also requested under this authorization, to the extent it is not already covered by an existing public right-of-way. [INSERT: legal description, acreage on Reclamation-administered land, and the legal status of the existing gravel road].', SCOPE + '\n' + ROAD)
...
rep(234, 'acquiring or exchanging federal land, which would not meet the development schedule;', 'acquiring or exchanging federal land, which would require a separate land-tenure action for a use that a use authorization can accommodate; constructing a new road alignment rather than improving the existing gravel road, which would create new disturbance on undisturbed land;',
    'Replaces a schedule-based reason (ties the purpose to the private development timeline) with a land-use reason, and adds the road alternative.')
R.delete_para(236, EO)

# ---- Section 3 ROW Location ----
R.insert_after(244, 'Access road segment, on Reclamation-administered land between the current end of Hart Lane and the western boundary of the CHIP parcels [INSERT: legal description and coordinates of the segment endpoints].', ROAD, like=244)
t = T(245)
...
b1='with a wing on each side of the corner (a single wing at Corner Crossing 2). At that standard each wing has legs of approximately 141 feet along the section lines and an outer face of approximately 200 feet, for approximately 0.23 acre per wing and approximately 1.6 acres for the seven wings [INSERT: confirm against the remapped project GIS and the corridor width required for the medium-voltage lines].'
a2='improved roadway.single wing of approximately 0.51 acre located entirely'
b2='improved roadway. Corner Crossing 2 is a single wing of approximately 0.23 acre located entirely'
assert a1 in t and a2 in t
R.edit(245, t.replace(a1,b1).replace(a2,b2), ACRES)
...
rep(310, 'T.19N., R.26E.', '[INSERT]')
rep(311, '2', '[INSERT]')
rep(312, 'Lot 4', '[INSERT: aliquot parts along the existing gravel road alignment]')
...
R.insert_after(331, 'Hart Lane access road segment. [INSERT: typical section, paved travel-way width, shoulders, right-of-way width, and drainage]. The segment would be built on the existing gravel road alignment and would be designed to Churchill County road standards for its connection to Hart Lane.', ROAD, like=331)
...
for i in (348, 356, 360):
    rep(i, 'Approximately 1.02 acres', 'Approximately 0.46 acre', ACRES if i == 348 else None)
rep(352, 'Approximately 0.51 acre', 'Approximately 0.23 acre')
rep(382, 'Transmission Corridor', 'Access Road Segment', SCOPE)
rep(383, 'Transmission corridor', 'Hart Lane access road segment')
R.edit(384, 'Reclamation-administered land between the current end of Hart Lane and the western boundary of the CHIP parcels [INSERT: legal description]')
R.edit(385, 'Paving and improvement of approximately 2,750 feet of an existing two-lane gravel road; no new alignment on Reclamation land [INSERT: right-of-way width and typical section]')
...
rep(415, 'Approximately 3.57 acres', 'Approximately 1.6 acres [INSERT: confirm against remapped KMZ]', ACRES)
rep(431, 'Transmission corridor', 'Hart Lane access road segment', SCOPE)
R.edit(432, 'Approximately 2,750 feet on the existing gravel road alignment [INSERT: width]')
R.edit(453, 'Within the crossing authorization boundaries (approximately 0.23 acre per wing); not additive to Table 3', 'Avoids double counting the wings as both permanent and temporary disturbance.')
